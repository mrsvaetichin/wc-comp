...
- **💬 Banter bonus:** Every ❤️ like on your banter post is **+1 point** toward your main standings. Anyone can 🟨 *book* a post they don't like; after **2 yellow cards** stack on a single post, the next dissenter can give a **🟥 red card** — that post is "sent off" and earns **0 points** regardless of its likes. **Likes & cards lock when the round changes** (group rounds md1/md2/md3 + a single ko bucket for knockouts), so old posts are frozen at whatever score they ended up on. The Banter race chart on the dashboard tracks this in real time, and the 💬 Board → Banter view shows the standings.
