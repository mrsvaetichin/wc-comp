...
- **Extras (tournament calls):** the 🃏 Extras tab holds whole-tournament predictions — champion (**20 pts**), top scorer, plus fun side questions — each worth its own points. Together they're tuned to ~10% of a top player's total, so they're a nice bonus but never league-deciding (the main game is the match picks). **All Extras lock the moment Round 1 locks** (30 min before the first match), and everyone's answers stay completely blind until that moment — then the crowd is revealed on every card and the whole set is sealed for the rest of the tournament.
- **💬 Banter bonus:** Each ❤️ on your banter post = **+1 point**, but **capped at +6 pts total** — that's ~2% of a top score, so banter is fun bragging rights and never decides the league. Anyone can 🟨 *book* a post they don't like; after **2 yellow cards** stack on a single post, the next dissenter can give a **🟥 red card** — that post is "sent off" and earns **0 points** regardless of its likes. **Likes & cards lock when the round changes** (group rounds `md1`/`md2`/`md3` + a single `ko` bucket for knockouts), so old posts freeze at whatever score they ended up on. The Banter race chart on the dashboard tracks live points (red-carded posts already excluded), and the 💬 Board → Banter view shows the standings.
